...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const initialSpeed = prefersReducedMotion ? 0 : speed
    ...
    const updateSize = () => {
      ...
      if (canvas.width !== nextWidth || canvas.height !== nextHeight) {
        ...
        resolutionRef.current = { width: nextWidth, height: nextHeight }
    ...
    const render = (now: number) => {
      ...
      const currentTime = (now - startTimeRef.current) * 0.001 * initialSpeed
      ...
      if (locations.u_resolution) context.uniform2f(locations.u_resolution, resolution.width, resolution.height)
    ...
    const visibilityObserver = new IntersectionObserver(
      ([entry]) => {
        ...
        if (isVisibleRef.current) {
          updateSize()
          if (initialSpeed !== 0 && !animationRef.current) {
            animationRef.current = requestAnimationFrame(render)
          }
          if (initialSpeed === 0) {
